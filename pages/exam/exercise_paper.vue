							...
							<view class="swiper-item uni-bg-red">
								<view class="list-box bg" v-if="questype && questype[item.questiontype]">
									<view v-if="questype[item.questiontype].questsort == 1">
										<view class="list-li">
											<h5 class="title">第{{current+1}}题 <span v-if="item.questionparent == 0">【{{questype[item.questiontype].questype}}】</span></h5>
										</view>
...
<script setup>
	...
	const getData = async () => {
		const data = await ajax({
			route:'exam-phone-exercise-paper',
		...
		for(let x in data.data.questype){
			if(questions[x]){
	...
	const submitPaper = async () => {
		const data = await ajax({
			route:'exam-phone-exercise-score',
			insertscore:1,
			token:token.value,
			sessionid:sessionid.value,
