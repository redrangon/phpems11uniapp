				...
				<view v-for="(section,sid) in sections" :key="sid">
					<view v-if="basic.basicknows[section['sectionid']]" :class="sid > 1 ? 'list-box bg top':'list-box bg'">
						...
						<view class="list-li" v-for="(know,kid) in basic.basicknows[section['sectionid']]" :key="kid">
							<navigator :url="'/pages/exam/lessonpaper?knowsid=' + know" hover-class="none">
							<h5 class="title">{{knows[know].knows}}</h5>
							<p class="intro">共{{sum(knows[know].knowsnumber)}}题<span class='pull-right' v-if="record[know]">上次做到第{{record[know].exernumber}}题，点击继续练习</span><span class='pull-right' v-else>开始练习</span></p>
							</navigator>
						</view>
					</view>
				</view>
