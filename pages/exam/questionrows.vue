				...
				<view class="list-box bg top">
					<view>
						<view class="list-li" v-for="(question,index) in questions" :key="index">
							<navigator :url="'/pages/exam/questionchild?questionid=' + question.qrid" hover-class="none">
								<view v-html="question.qrquestion"></view>
							</navigator>
						</view>
					</view>
				</view>
...
<script setup>
	...
	const getData = async() => {
		const data = await ajax({
			route:'exam-phone-questions-questionrows',
	...
	const questionsearch = (e) => {
		keyword.value = e.detail.value.keyword;
		questions.value = [];
		current.value = 1;
		getData();
	}
