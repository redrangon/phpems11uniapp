		...
		<view class="page-tabs">
				...
				<view class="list-box bg top">
					<view v-if="allnumber >= 1">
						...
						<view class="list-li rightanswer">
							<view class="rows">
								<view v-if="question.questionanswer.length >= 20">
									<view class="intro">
			...
			<view class="page-footer" v-if="allnumber >= 1">
				<view class="pagination">
					<view class="col-2" @click="delrecord(record.recordid)">
...
<script setup>
	...
	const getData = async () => {
		const data = await ajax({
			route:'exam-phone-record-records'
		});
	}
	const getQuestion = async () => {
		const data = await ajax({
			route:'exam-phone-record-ajax-questions',
			page:number.value
		});
	...
	const delrecord = async (id) => {
		const data = await ajax({
			route:'exam-phone-record-ajax-delrecord',
			recordid:id
		})
